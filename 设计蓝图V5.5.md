...
页面上有一个“获取系统状态”按钮。点击后，通过WebSocket发送一个JSON RPC请求：{"jsonrpc": "2.0", "method": "system.reboot", "id": 1}。在设备的串口能看到Task_Worker收到该命令的日志。
...
##### UI：使用Bootstrap 5构建响应式布局（卡片、信息列表、操作按钮、日志控制台）能响应式地适应桌面和移动端。

##### 数据加载: ：页面加载时，通过WebSocket发送settings.get和wifi.scan(或其他获取状态的命令)，并用返回结果填充UI。

##### 实时更新：监听system.stateUpdate通知，实时更新“运行时间”、“堆内存”等卡片信息。

##### 操作按钮：实现“重启”、“恢复出厂设置”按钮，绑定system.reboot和system.factoryReset命令，并必须使用Bootstrap Modal进行二次确认。
实现“调试模式”开关，绑定settings.saveDebug命令（需要在API_SPEC.md和Sys_Tasks.cpp中新增此命令），并用Toast提示保存结果。
...
##### 为了让日志更具可读性，建议在调用 .log() 时，第一个参数采用统一的模块名标签，形成如 [WiFi], [Settings], [ERROR] 等格式化的日志条目。
...
##### 明确OTA分区，otadata 分区用于记录哪个是当前启动的APP分区，保证即使升级失败，系统也能回滚到上一个正常版本。
